...
function grad(f) {
    ...
    return function (x, dy) {
        util.assert(x instanceof tensor_1.Tensor, function () { return 'The x passed in grad(f)(x) must be a tensor'; });
        util.assert(dy == null || dy instanceof tensor_1.Tensor, function () { return 'The dy passed in grad(f)(x, dy) must be a tensor'; });
...
function grads(f) {
    ...
    return function (args, dy) {
        util.assert(Array.isArray(args) && args.every(function (arg) { return arg instanceof tensor_1.Tensor; }), function () { return 'The args passed in grads(f)(args) must be an array of tensors'; });
        util.assert(dy == null || dy instanceof tensor_1.Tensor, function () { return 'The dy passed in grads(f)(args, dy) must be a tensor'; });
        return environment_1.ENV.engine.tidy(function () {
...
function variableGrads(f, varList) {
    ...
    util.assert(varList.length > 0, function () {
        return "variableGrads() expects at least one of the input variables to be " +
