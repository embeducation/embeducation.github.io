...
jasmine_util_1.describeWithFlags('relu', test_util_1.ALL_ENVS, function () {
    ...
    it('does nothing to positive values', function () {
        var a = tf.scalar(1);
        var result = tf.relu(a);
        test_util_1.expectArraysClose(result, [1]);
    });
...
jasmine_util_1.describeWithFlags('relu', test_util_1.WEBGL_ENVS, function () {
    it('works with squarification for prime number length vector', function () {
        var maxTextureSize = tf.ENV.get('WEBGL_MAX_TEXTURE_SIZE');
        tf.ENV.set('WEBGL_MAX_TEXTURE_SIZE', 5);
        var a = tf.tensor1d([1, -2, 5, -3, -1, 4, 7]);
        var result = tf.relu(a);
        tf.ENV.set('WEBGL_MAX_TEXTURE_SIZE', maxTextureSize);
        test_util_1.expectArraysClose(result, [1, 0, 5, 0, 0, 4, 7]);
    });
});
...
jasmine_util_1.describeWithFlags('logSigmoid', test_util_1.ALL_ENVS, function () {
    ...
    it('gradients: Scalar', function () {
        ...
        var aVal = a.arraySync();
        var y = 1 / (1 + Math.exp(aVal));
        test_util_1.expectArraysClose(da, [dyVal * y]);
    });
    it('gradients: Tensor1D', function () {
        var a = tf.tensor1d([1, 2, -3, 5]);
        var aVals = a.arraySync();
...
jasmine_util_1.describeWithFlags('sqrt', test_util_1.ALL_ENVS, function () {
    it('sqrt', function () {
        ...
        var r = tf.sqrt(a);
        test_util_1.expectArraysClose(r, [Math.sqrt(2), Math.sqrt(4)]);
    });
...
jasmine_util_1.describeWithFlags('rsqrt', test_util_1.ALL_ENVS, function () {
    it('rsqrt', function () {
        var a = tf.tensor1d([2, 4]);
        var r = tf.rsqrt(a);
        test_util_1.expectArraysClose(r, [1 / Math.sqrt(2), 1 / Math.sqrt(4)]);
    });
...
jasmine_util_1.describeWithFlags('log', test_util_1.ALL_ENVS, function () {
    it('log', function () {
        ...
        var r = tf.log(a);
        test_util_1.expectArraysClose(r, [Math.log(1), Math.log(2)]);
    });
    it('log 6D', function () {
...
jasmine_util_1.describeWithFlags('log1p', test_util_1.ALL_ENVS, function () {
    it('log1p', function () {
        var a = tf.tensor1d([1, 2]);
        var r = tf.log1p(a);
        test_util_1.expectArraysClose(r, [Math.log1p(1), Math.log1p(2)]);
    });
...
jasmine_util_1.describeWithFlags('ceil', test_util_1.ALL_ENVS, function () {
    it('basic', function () {
        ...
        var r = tf.ceil(a);
        test_util_1.expectArraysClose(r, [2, 3, -1]);
    });
    it('propagates NaNs', function () {
...
jasmine_util_1.describeWithFlags('floor', test_util_1.ALL_ENVS, function () {
    it('basic', function () {
        var a = tf.tensor1d([1.5, 2.1, -1.4]);
        var r = tf.floor(a);
        test_util_1.expectArraysClose(r, [1, 2, -2]);
    });
    it('propagates NaNs', function () {
        var a = tf.tensor1d([1.5, NaN, -1.4]);
        var r = tf.floor(a);
...
jasmine_util_1.describeWithFlags('sign', test_util_1.ALL_ENVS, function () {
    it('basic', function () {
        ...
        var r = tf.sign(a);
        test_util_1.expectArraysClose(r, [1, 0, 0, -1]);
    });
    it('propagates NaNs', function () {
        var a = tf.tensor1d([1.5, NaN, -1.4]);
        var r = tf.sign(a);
...
jasmine_util_1.describeWithFlags('exp', test_util_1.ALL_ENVS, function () {
    it('exp', function () {
        ...
        var r = tf.exp(a);
        test_util_1.expectArraysClose(r, [Math.exp(1), Math.exp(2), 1]);
    });
...
jasmine_util_1.describeWithFlags('expm1', test_util_1.ALL_ENVS, function () {
    it('expm1', function () {
        var a = tf.tensor1d([1, 2, 0]);
        var r = tf.expm1(a);
        test_util_1.expectArraysClose(r, [Math.expm1(1), Math.expm1(2), Math.expm1(0)]);
    });
...
jasmine_util_1.describeWithFlags('packed clip', test_util_1.PACKED_ENVS, function () {
    ...
    it('using extreme values', function () {
        var a = tf.tensor1d([3, -1, 0, 100, -7, 2]);
        var result = tf.clipByValue(a, Number.NEGATIVE_INFINITY, Number.POSITIVE_INFINITY);
        test_util_1.expectArraysClose(result, [3, -1, 0, 100, -7, 2]);
        result = tf.clipByValue(a, Number.MIN_VALUE, Number.MAX_VALUE);
        test_util_1.expectArraysClose(result, [3, Number.MIN_VALUE, Number.MIN_VALUE, 100, Number.MIN_VALUE, 2]);
    });
    it('should work for scalars', function () {
        var a = tf.scalar(-4);
        var min = -1;
        var max = 50;
        var result = tf.clipByValue(a, min, max);
        test_util_1.expectArraysClose(result, [min]);
    });
...
jasmine_util_1.describeWithFlags('clip', test_util_1.ALL_ENVS, function () {
    ...
    it('derivative: 1D tensor', function () {
        var min = -1;
        var max = 2;
        var x = tf.tensor1d([3, -2, 1]); // Only 1 is not clipped.
        var dy = tf.tensor1d([5, 50, 500]);
        var gradients = tf.grad(function (x) { return x.clipByValue(min, max); })(x, dy);
        expect(gradients.shape).toEqual(x.shape);
        expect(gradients.dtype).toEqual('float32');
    ...
    it('clip(x, eps, 1-eps) never returns 0 or 1', function () {
        var min = tf.ENV.get('EPSILON');
        var max = 0.5;
        var res = tf.clipByValue([0, 1], min, max).dataSync();
...
jasmine_util_1.describeWithFlags('atanh', test_util_1.ALL_ENVS, function () {
    ...
    it('accepts a tensor-like object', function () {
        ...
        test_util_1.expectArraysClose(result, [Math.atanh(0.2)]);
    });
    it('throws for string tensor', function () {
...
jasmine_util_1.describeWithFlags('erf', test_util_1.ALL_ENVS, function () {
    ...
    it('accepts a tensor-like object', function () {
        var result = tf.erf(1);
        test_util_1.expectArraysClose(result, [0.8427008]);
    });
    it('throws for string tensor', function () {
