...
jasmine_util_1.describeWithFlags('concat1d', test_util_1.ALL_ENVS, function () {
    ...
    it('TensorLike Chained 3 + 5', function () {
        var a = tf.tensor1d([3]);
        var b = [5];
...
jasmine_util_1.describeWithFlags('concat2d', test_util_1.ALL_ENVS, function () {
    ...
    it('TensorLike [[3]] + [[5]], axis=0', function () {
        var axis = 0;
        var a = [[3]];
        var b = [[5]];
        var result = tf.concat2d([a, b], axis);
    ...
    it('TensorLike Chained [[3]] + [[5]], axis=0', function () {
        var axis = 0;
        var a = tf.tensor2d([3], [1, 1]);
        var b = [[5]];
        var result = a.concat([b], axis);
        var expected = [3, 5];
        expect(result.shape).toEqual([2, 1]);
        test_util_1.expectArraysClose(result, expected);
    });
    ...
    it('concat zero-sized tensors', function () {
        var a = tf.tensor2d([], [0, 5]);
        var b = tf.tensor2d([], [0, 5]);
        var c = tf.tensor2d([], [0, 5]);
        var res = tf.concat([a, b, c], /* axis */ 0);
        expect(res.shape).toEqual([0, 5]);
        test_util_1.expectArraysEqual(res, []);
        var res2 = tf.concat([a, b, c], /* axis */ 1);
        expect(res2.shape).toEqual([0, 15]);
        test_util_1.expectArraysEqual(res2, []);
    });
});
jasmine_util_1.describeWithFlags('concat3d', test_util_1.ALL_ENVS, function () {
    ...
    it('TensorLike concat axis=0', function () {
        var tensor1 = [[[1, 11, 111], [2, 22, 222]]];
        var tensor2 = [[[5, 55, 555], [6, 66, 666]], [[7, 77, 777], [8, 88, 888]]];
        var values = tf.concat3d([tensor1, tensor2], 0);
        expect(values.shape).toEqual([3, 2, 3]);
        test_util_1.expectArraysClose(values, [
            1, 11, 111, 2, 22, 222, 5, 55, 555, 6, 66, 666, 7, 77, 777, 8, 88, 888
    ...
    it('TensorLike Chained concat axis=0', function () {
        var tensor1 = tf.tensor3d([1, 11, 111, 2, 22, 222], [1, 2, 3]);
        var tensor2 = [[[5, 55, 555], [6, 66, 666]], [[7, 77, 777], [8, 88, 888]]];
        var values = tensor1.concat([tensor2], 0);
        expect(values.shape).toEqual([3, 2, 3]);
        test_util_1.expectArraysClose(values, [
            1, 11, 111, 2, 22, 222, 5, 55, 555, 6, 66, 666, 7, 77, 777, 8, 88, 888
        ]);
    ...
    it('concat a large number of tensors, axis=0', function () {
        var tensors = [];
        var expected = [];
        for (var i = 0; i < 100; i++) {
        ...
        var axis = 0;
        var res = tf.concat(tensors, axis);
        expect(res.shape).toEqual([100]);
        expect(res.dtype).toBe('float32');
        test_util_1.expectArraysClose(res, expected);
    });
    it('concat a large number of tensors, axis=1', function () {
        var tensors = [];
        var expected = [];
        for (var i = 0; i < 100; i++) {
    ...
    it('accepts a tensor-like object', function () {
        var tensor1 = [[[1, 2, 3]]]; // 1x1x3
        var tensor2 = [[[4, 5, 6]]]; // 1x1x3
        var values = tf.concat3d([tensor1, tensor2], 0);
        expect(values.shape).toEqual([2, 1, 3]);
        test_util_1.expectArraysClose(values, [1, 2, 3, 4, 5, 6]);
    });
    it('concat tensors with 0 in their shape', function () {
        var tensor1 = tf.tensor3d([1, 2, 3, 4, 5, 6], [2, 3, 1]);
        var tensor2 = tf.tensor3d([], [0, 3, 1]);
        var values = tf.concat3d([tensor1, tensor2], 0);
...
jasmine_util_1.describeWithFlags('concat throws for non-tensors', test_util_1.ALL_ENVS, function () {
    ...
    it('accepts a tensor-like object', function () {
        var tensor1 = [[[1, 2, 3, 4]]]; // 1x1x4
        var tensor2 = [[[4, 5, 6, 7]]]; // 1x1x4
