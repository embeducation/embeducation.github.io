...
function showVizualization(words = words_HP, dictX = dictX_HP, dictY = dictY_HP, dictZ = dictZ_HP) {
	...
	var getRandomText = function() {
		...
		return Array
			...
			.map(function(){
				...
				.from({length: 1})
				.map(function() {
					var i = getRandomInt(current_words.length);
					return current_words[i];
				})
				.join(' ');
			})
			.join('\n');
